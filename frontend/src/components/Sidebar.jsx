...
import { Home, ClipboardList, Utensils, Dumbbell, Settings, LogOut } from 'lucide-react';
    ...
    return (
        <div className="h-screen w-64 bg-white dark:bg-slate-800 border-r border-slate-100 dark:border-slate-700 flex flex-col p-6 fixed left-0 top-0 z-20">
            ...
            <div className="flex items-center gap-3 px-2 mb-10">
                ...
                <span className="text-xl font-bold text-slate-800 dark:text-white tracking-tight">Nutrivitals</span>
            </div>
                ...
                {menuItems.map((item) => {
                    ...
                    return (
                        <Link
                            ...
                            to={item.path}
                        >
                            <div className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${isActive ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400 font-medium shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50 hover:text-slate-900 dark:hover:text-white'}`}>
                                <item.icon size={20} />
